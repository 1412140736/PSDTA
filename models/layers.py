...
from models.graphomer import GraphTransformerLayer,MultiHeadAttentionLayer
...
class DrugProteinConv(MessagePassing):
    ...
    def __init__(
        ...
        **kwargs, #接受其他额外参数，并传递给父类 MessagePassing 进行处理
    ):
        ...
        if edge_dim is not None:
            ...
            self.lin_edge = self.register_parameter('lin_edge', None)  #确保模型有lin_edge这个占位符。防止在没有边特征的时候访问该属性时出现异常。
    ...
    def forward(self, drug_x, clique_x, clique_batch, residue_x, edge_index: Adj):
        ...
        query = self.lin_query(drug_x).view(-1, H, aC)  #这里的drug_x就是公式里的L_BALL
        ...
        drug_out = self.drug_out_norm(drug_out)  #公式里的M_REGION*P
