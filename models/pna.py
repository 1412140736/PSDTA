...
class PNAConv(MessagePassing):
    ...
    def __init__(self, in_channels: int, out_channels: int,
                 ...
                 act_kwargs: Optional[Dict[str, Any]] = None, #激活函数的参数
                 divide_input: bool = False, **kwargs): #是否将输入特征按塔的数量进行拆分。

        aggr = DegreeScalerAggregation(aggregators, scalers, deg) #aggregators是聚合器、scalers是缩放器、deg表示结点度向量
        ...
        for _ in range(towers):
            modules = [Linear((3 if edge_dim else 2) * self.F_in, self.F_in)]  #如果有边特征，输入维度为 (3 * self.F_in) 否则为 (2 * self.F_in)
            for _ in range(pre_layers - 1):
                modules += [activation_resolver(act, **(act_kwargs or {}))] #激活层
