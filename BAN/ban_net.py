...
from models.sub_graph import GIBGCN, Discriminator
...
class net(torch.nn.Module):
    def __init__(self, mol_deg, prot_deg,
                 # MOLECULE
                 mol_in_channels=43, prot_in_channels=21, prot_evo_channels=1280,
                 hidden_channels=200,
        ...
        self.prot_degree = MLP([1, hidden_channels * 2, hidden_channels], out_norm=True)
        ...
        self.prot_two = MLP([hidden_channels * 2, hidden_channels * 2, hidden_channels], out_norm=True)
        ...
        self.protein_subgraph = GIBGCN(hidden_channels, hidden_channels)  #注意写一下reset
        self.protein_discriminator = Discriminator(hidden_channels)
        self.discriminator_inner_loop = 20
        self.discriminator_lr = 0.01

        self.atom_sub_line = MLP([hidden_channels * 2, hidden_channels * 2, hidden_channels], out_norm=True)
        self.clique_pro_line = MLP([hidden_channels * 2, hidden_channels * 2, hidden_channels], out_norm=True)

    def reset_parameters(self):
        self.atom_feat_encoder.reset_parameters()
        self.prot_evo.reset_parameters()
        self.prot_aa.reset_parameters()
        self.atom_sub_line.reset_parameters()
        self.clique_pro_line.reset_parameters()
        self.protein_subgraph.reset_parameters()
        self.protein_discriminator.reset_parameters()
        self.prot_degree.reset_parameters()
        self.prot_two.reset_parameters()
    ...
    def forward(self,
        ...
        #residue_x = residue_x[:, :32]  #消融实验时用到了

        # PROTEIN Featurize
        # residue_degree = residue_degree.unsqueeze(1).to(residue_x.dtype)
        # residue_degree = self.prot_degree(residue_degree)
        residue_x = self.prot_aa(residue_x) + self.prot_evo(residue_evo_x)  #+ residue_degree
        ...
        for idx in range(self.total_layer):
            ...
            if idx == self.total_layer-1:
                self.atom_x_subgraph = atom_x
                self.residue_x_subgraph = residue_x
            ...
            ## Cluster protein residues  要对蛋白质进行聚类
            ...
            s = self.cluster[idx](residue_x, dropped_residue_edge_index)  #聚类，形状没变，聚合了特征
            ...
            cluster_x = self.prot_norms[idx](cluster_x)  #归一化
            ...
            clique_x, cluster_x, inter_attn = self.inter_convs[idx](drug_x, clique_x, clique_batch, cluster_x,
                                                                    p2m_edge_index)
            ...
            atom_x = atom_x + F.relu(self.atom_lins[idx](
                scatter(clique_x[col], row, dim=0, dim_size=atom_x.size(0), reduce='mean')))  # clique -> atom
        ...
        mol_pool_feat = F.dropout(mol_pool_feat, self.dropout, training=self.training)
        prot_pool_feat = F.dropout(prot_pool_feat, self.dropout, training=self.training)


        mol_feature = self.atom_x_subgraph
        prot_feature = self.residue_x_subgraph

        mol_node_embs = []
        num_graph = torch.max(mol_batch)
        for graph_id in range(num_graph+1):
            mol_node_emb = mol_feature[mol_batch == graph_id]
            mol_node_emb = torch.mean(mol_node_emb, dim=0, keepdim=True)
            mol_node_embs.append(mol_node_emb)

        prot_feature = prot_feature.repeat(num_graph+1, 1, 1)
        mol_node_embs = torch.stack(mol_node_embs)
        mol_node_embs = mol_node_embs.expand(-1, prot_feature.size(1), -1)  # 形状为 (4, n, dim)
        prot_mol_feature = torch.cat([prot_feature, mol_node_embs], dim=-1)

        subgraph_out, protein_subgraph_emb, protein_graph_embedding, protein_pos_penalty, assignment = self.protein_subgraph(
            prot_mol_feature, residue_edge_index, prot_batch, prot_feature)

        # bilinear_weight = torch.randn(hidden_channels, hidden_channels)  # 可学习参数，形状为 (dim, dim)
        # all_prot_feature = torch.einsum('bd,dc->bc', prot_pool_feat, bilinear_weight) * subgraph_out  # (batch, dim)

        all_prot_feature = self.prot_two(torch.cat([prot_pool_feat, subgraph_out], dim=-1))

        mol_prot_feat = torch.cat([mol_pool_feat, all_prot_feature], dim=-1)
        ...
        if mode=='train':
            ...
            return reg_pred, spectral_loss, ortho_loss, cluster_loss, protein_pos_penalty, protein_mi_loss
        else:
            return reg_pred, spectral_loss, ortho_loss, cluster_loss, assignment, residue_score
    ...
    def freeze_backbone_optimizers(self, finetune_module, weight_decay, learning_rate, betas, eps,
                                   amsgrad):  ## only for fineTune Pretrain model
        """
