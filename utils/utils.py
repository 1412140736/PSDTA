...
def compute_pna_degrees(train_loader):
    ...
    for data in tqdm(train_loader):
        # mol
        mol_d = degree(data.mol_edge_index[1], num_nodes=data.mol_x.shape[0], dtype=torch.long) #mol_d 是一个一维张量，表示每个节点的度数
        mol_max_degree = max(mol_max_degree, int(mol_d.max())) #统计最大度数
        # clique
        try:
            clique_d = degree(data.clique_edge_index[1], num_nodes=data.clique_x.shape[0], dtype=torch.long) #计算团簇的度
        except RuntimeError:
            ...
            break
        clique_max_degree = max(clique_max_degree, int(clique_d.max())) #统计团簇最大度数
        # protein
        prot_d = degree(data.prot_edge_index[1], num_nodes=data.prot_node_aa.shape[0], dtype=torch.long) #计算氨基酸的度
        prot_max_degree = max(prot_max_degree, int(prot_d.max())) #统计氨基酸最大度数

    # Compute the in-degree histogram tensor 初始化三个张量，分别用于记录分子、团簇、和蛋白质图的节点度数直方图。
    ...
    for data in tqdm(train_loader):
        # mol  对每个分子图的节点度数进行计数，并更新分子图的度数直方图张量 mol_deg。
    ...
    return mol_deg, clique_deg, prot_deg #结点度的直方图。反应不同度的出现频率
...
def extract_data_from_files(file_list):
    all_data_tuples = []
    seen_proteins = set()  # 用于存储已经出现过的Protein
    ...
    for file in file_list:
        ...
        # 确保文件中包含必要的列
        required_columns = ['ID', 'Protein', 'Target_Chain']
        if not all(col in df.columns for col in required_columns):
            raise ValueError(f"CSV文件 {file} 中必须包含列：{', '.join(required_columns)}")

        # 创建一个包含每行数据元组的列表，并检查Protein是否重复
        for _, row in df.iterrows():
            protein = row['Protein']
            if protein not in seen_proteins:
                seen_proteins.add(protein)
                data_tuple = (row['ID'], protein, row['Target_Chain'])
